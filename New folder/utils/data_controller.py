...
def load_local_dataset(
    ...
    """
    Load dataset from local files (for phase3 where images are already stored locally).
    
    Args:
        metadata_df: DataFrame with columns ['img_id', 'edit_prompt']
        batch_size: Number of images to load
        start_id: Starting image ID
        orig_dir: Directory containing original images (e.g., datasets/phase3/src/orig)
        edited_dir: Directory containing edited images (e.g., datasets/phase3/src/edited)
        exclude_current: Whether to exclude the start_id from results
    
    Returns:
        Tuple of (img_id_list, edit_prompts, orig_paths, edited_paths)
    """
    ...
    batch_metadata = (
        metadata_df[mask]
        .sort_values("img_id")
        .head(batch_size)[["img_id", "edit_prompt"]]
    )
    ...
    for i, row in batch_metadata.iterrows():
        ...
        orig_path = orig_dir / f"{img_id}_orig.png"
        edited_path = edited_dir / f"{img_id}_edited.png"
        ...
        if not (exclude_current and img_id == start_id):
            ...
            img_src_orig_paths.append(orig_path)
            img_src_edited_paths.append(edited_path)
...
def load_dataset(
    ...
    """
    Load dataset by downloading images from URLs (for phase1a/1b with URL columns).
    
    Args:
        metadata_df: DataFrame with columns ['img_id', 'edit_prompt', 'orig_img_url', 'edited_img_url']
        batch_size: Number of images to load
        start_id: Starting image ID
        orig_dir: Directory to save original images
        edited_dir: Directory to save edited images
        exclude_current: Whether to exclude the start_id from results
        no_cache: Whether to re-download even if files exist
